...
import 'models/co_result_dto_tabela.dart';
...
class LoanRepository {
  ...
  Future<Either<LoanException, CoResultDtoJornadaI>> insertJornada(
      {required RequestJornadaDto request}) async {
    try {
      var resp = await _clientAuth.post('${kBaseUrl}appcommon/jornada/insert',
...
Future<Either<LoanException, File>> baixarResumoJornada({
  ...
  try {
    ...
    // Obter o diretório da pasta Downloads ou local adequado
    Directory? directory;
    if (Platform.isAndroid) {
      directory = await getExternalStorageDirectory(); // Diretório externo
    } else if (Platform.isIOS) {
      directory = await getApplicationSupportDirectory(); // Local interno no iOS
    }

    if (directory == null) {
      return Left(LoanExceptionGeneric(message: "Não foi possível acessar o diretório."));
    }
  ...
  Future<Either<LoanException, CoResultDtoJornadaEtapa>>
      filterJornadaEtapaProposta({required int jornadaId}) async {
    try {
      var resp = await _clientAuth
          .post('${kBaseUrl}appcommon/jornadaetapa/filter', data: {
        "classname": "CoQueryObjectDynamic",
